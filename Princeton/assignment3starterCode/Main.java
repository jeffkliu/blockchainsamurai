...
public class Main {
   ...
   public static void main(String[] args) throws NoSuchProviderException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
	...
	genesis.finalize();
    System.out.println("We're at Genesis Block now: ");
    ...
    for(int i =0 ; i < 100; i++)
    {
        Block newBlock = new Block(genesis.getHash(), pk_scrooge.getPublic());
        Tx testtx = new Tx();
        testtx.addInput(genesis.getCoinbase().getHash(), 0);
        testtx.addOutput(25, pk_scrooge.getPublic());
        testtx.signTx(pk_scrooge.getPrivate(),0);

        newBlock.addTransaction(testtx);
        newBlock.finalize();

        System.out.println("BlockTesttt Added ok: " + bh.processBlock(newBlock));
     ...
     * Create alternative block, block2 from scrooge with tx scrooge -> scrooge
         */
    Block blockextra = new Block(genesis.getHash(), pk_scrooge.getPublic());

        // new TX: scrooge pays 25 coins to scrooge
        Tx tx6 = new Tx();
        tx6.addInput(genesis.getCoinbase().getHash(), 0);  //25
        tx6.addOutput(5, pk_scrooge.getPublic());
        tx6.addOutput(10, pk_scrooge.getPublic());
        tx6.addOutput(10, pk_scrooge.getPublic());
        tx6.signTx(pk_scrooge.getPrivate(), 0);

    blockextra.addTransaction(tx2);
    blockextra.finalize();
    
    System.out.println("Blockextra Added ok: " + bh.processBlock(blockextra));
     ...
     * Create alternative block, block2 from scrooge with tx scrooge -> scrooge
         */
    Block blockextra1 = new Block(block3.getHash(), pk_scrooge.getPublic());

        // new TX: scrooge pays 25 coins to scrooge
        Tx tx7 = new Tx();
        tx7.addInput(genesis.getCoinbase().getHash(), 0);  //25
        tx7.addOutput(10, pk_scrooge.getPublic());
        tx7.signTx(pk_scrooge.getPrivate(), 0);

    blockextra1.addTransaction(tx7);
    blockextra1.finalize();
    
    System.out.println("Blockextra1 Added ok: " + bh.processBlock(blockextra1));
